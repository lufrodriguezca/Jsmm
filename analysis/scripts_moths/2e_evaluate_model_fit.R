...
for(mname in c("moths")){
  ...
  #load("models/unfitted_model_moths.RData")
  sel = 1:m$ns
  ...
  for(k in 1:4){
    if(k == 1){
      fun = function(m,captures) number_of_captures(m, captures)
      xlab = ""
      ylab = "total number of captures"
    }
    if(k == 2){
      fun = function(m, captures) mean_captures(m, captures)
      xlab = ""
      ylab = "mean number of captures per individual"
    }
    if(k == 3){
      fun = function(m,captures) mean_time_to_capture(m,captures)
      xlab = ""
      ylab = "mean time to capture"
    }
    if(k == 4){
      fun = function(m, captures) mean_distance_between_captures(m, captures)
      xlab = ""
