  ...
  for(mname in c("moths")){
    ...
    for(type in types){
      ...
      for(repl in 1:nrepl){
        ...
        if(type=="prior"){
          pars = Jsmm::simulate_from_prior(m)
        }
        max_dt = NULL
        secondary_release = FALSE
        releases = m[["releases"]]
        m2 = Jsmm::simulate_movement(m = m, pars = pars, releases = releases,
      ...
      save(PPcaptures,file = paste0("models/predicted_captures_",mname,"_",type,"_nrepl_",nrepl,".RData"))
    }
  }
}
