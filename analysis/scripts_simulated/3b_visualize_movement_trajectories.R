...
for(method in c("ICP","CCP")){
  ...
  ni = 1#round(10000/m[["ns"]]) #number of individuals per species
  ...
  m = simulate_trajectories(m = m, pars = pars, releases = releases,
                        max_dt = max_dt)
  ...
  customize_plot$trajectory_color = rep("black", m[["ns"]])
  customize_plot$trajectory_lwd = 0.2
  plot_sp = 1:m[["ns"]]

  show(plot_trajectories(m = m, customize_plot = customize_plot, plot_sp = plot_sp  ))
}