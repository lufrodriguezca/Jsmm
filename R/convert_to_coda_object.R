                               ...
                               Beta = TRUE, Gamma = TRUE, rho = TRUE, SI = TRUE, li = TRUE)
{
  ...
  for (chain in 1:nChains){
    ...
    if(Beta){
      ...
      postBeta[[chain]] = coda::mcmc(tmp, thin = thin, start = start1, end = end1)
    }
    ...
    if (Gamma){
      ...
      postGamma[[chain]] = coda::mcmc(tmp, thin = thin, start = start1, end = end1)
    }
    ...
    if (SI){
      ...
      postSI[[chain]] = coda::mcmc(tmp, thin = thin, start = start1, end = end1)
    }

    if (rho){
      postRho[[chain]] = coda::mcmc(unlist(lapply(postList, function(a) a$rho)), thin = thin,
    ...
    if (li){
      ...
      postLi[[chain]] = coda::mcmc(tmp, thin = thin, start = start1, end = end1)
    }
  }
  ...
  if (Beta){
    mpost$Beta = coda::mcmc.list(postBeta)
  }
  if (Gamma){
    mpost$Gamma = coda::mcmc.list(postGamma)
  }
  if (rho){
    mpost$rho = coda::mcmc.list(postRho)
  }
  if (SI){
    mpost$SI = coda::mcmc.list(postSI)
  }
  if (li){
    mpost$li = coda::mcmc.list(postLi)
  }
  return(mpost)
}
