...
simulate_from_prior = function(m){
  ...
  SI = MCMCpack::riwish(v = nu, S = PSI)

  tr = m[["X"]][["trait"]]

  nt = dim(tr)[2]

  Z = matrix(MASS::mvrnorm(1, mu = muZ, Sigma = VZ), ncol = np, byrow = TRUE)

  M = tr%*%Z
  em = matrix(t(M), nrow = 1)

  THETAS = matrix(MASS::mvrnorm(1, mu = em, Sigma = kronecker(rho*C + (1 - rho)*Ins, SI)), nrow = 1)
