...
plot_domain = function(domain, customize_plot = NULL){
  ...
  (ggplot(data = domain$polygon)
    + geom_sf(data = domain$polygon[which(domain$polygon$id > 0), ], aes(fill = factor(id)),
    ...
    + geom_sf(data = domain$polygon[which(domain$polygon$id_location != 0), ], fill = customize_plot$site_col , color = customize_plot$siteb_col,
              lwd = customize_plot$lwd_poly)
    + geom_text(data = center_location, aes(x = x_centroid, y = y_centroid, label = id_location), size = customize_plot$site_text_size,
