...
jsmm_add_triangulation = function(domain,  max_t_area, min_t_angle){
  ...
  for(e in 1:nele){
    ...
    dom_polygon[[e]] = st_polygon(list(as.matrix(rbind(ele_r, ele_r[1, ]))))
  }

  dom_polygon = st_sf(id = rep(0, nele), id_location = rep(0, nele), geometry = st_sfc(dom_polygon))
  dom_polygon = st_set_crs(dom_polygon, st_crs(domain))

  options(warn = -1)
  c_points = st_centroid(dom_polygon)
  options(warn = 0)

  p   = domain[which(domain[["id"]] != 1 ), ]

  pid = st_covered_by(c_points, p)
