...
plot_trajectories = function(m = m, plot_sp = NULL, customize_plot = NULL){
  ...
  if(nrow(trajectories) > 0){
    ...
    for(s in plot_sp){
      ...
      if(length(indList)>0){
        ...
        trajs = list()
        trajs[[1]] =  st_multilinestring(traj_line)
        trajs = st_sf(sp = s, geometry = st_sfc(trajs))
      }
  ...
  if(ncol(trajectory) > 0){
    (d + geom_sf(data=traj[which(traj$sp %in% plot_sp ), ], color = customize_plot$trajectory_color[plot_sp], lwd = customize_plot$trajectory_lwd))
  }else{
    d
  }
}
