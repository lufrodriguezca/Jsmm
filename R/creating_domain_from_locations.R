...
creating_domain_from_locations = function(lc, lc_label = NA, buffer_x = c(0,0), buffer_y = c(0, 0), nnod_side_x, nnod_side_y,
  ...
  border_dom = list()
  border_dom[[1]] = sf::st_polygon(list(polycoor))
  border_dom = sf::st_sf(id = 1, id_location = 0, geometry = sf::st_sfc(border_dom))

  border_dom_repeat = list()
  border_dom_repeat[[1]] = sf::st_polygon(list(polycoor))
  border_dom_repeat = sf::st_sf(id = 2, id_location = 0, geometry = sf::st_sfc(border_dom_repeat))
  ...
  for(i in 1:nrow(lc)){
    ...
    sites_l[[i]] = sf::st_polygon(list(poly_trap))
  }

  s = sf::st_sf(id = rep(2, nrow(lc)), id_location = 1:nrow(lc),  geometry = sf::st_sfc(sites_l))

  if( int_buffer && ((minx - buffer_x[1]/2) > min(sf::st_coordinates( s)[,1]) || (minx - buffer_x[1]/2) == min(sf::st_coordinates( s)[, 1]) ||
                     (maxx + buffer_x[2]/2) < max(sf::st_coordinates( s)[,1]) || (maxx + buffer_x[2]/2) == max(sf::st_coordinates( s)[, 1]) ||
                     (miny - buffer_y[1]/2) > min(sf::st_coordinates( s)[,2]) || (miny - buffer_y[1]/2) == min(sf::st_coordinates( s)[, 2]) ||
                     (maxy + buffer_y[2]/2) < max(sf::st_coordinates( s)[,2]) || (maxy + buffer_y[2]/2) == max(sf::st_coordinates( s)[, 2]))){
    int_buffer = FALSE
  ...
  if(int_buffer){
    ...
    border_domb = list()
    border_domb[[1]] = sf::st_polygon(list(polycoorb))
    border_domb = sf::st_sf(id = 2, id_location = 0, geometry = sf::st_sfc(border_domb))
